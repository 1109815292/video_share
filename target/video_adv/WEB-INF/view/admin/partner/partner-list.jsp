...
<script>
    ...
    layui.use(['form', 'layer','util', 'table'], function () {
        ...
        dataGrid = table.render({
            elem: '#tableList'
            , toolbar: '#toolbar'
            , url: '${contextPath}/admin/partner/getPartnerData'
            , cellMinWidth: 70
            , id: 'id'
            , cols: [[
        ...
        table.on('toolbar(tableList)', function (obj) {
            ...
            switch (obj.event) {
                case 'refresh'://刷新
                    ...
                    break;
                case 'new'://新增
                    x_admin_show('添加合伙人', '<%=path%>/admin/partner/partnerUserEdit');
                    break
            }
        });
        ...
        table.on('tool(tableList)', function (obj) {
            var data = obj.data;
            if (obj.event === 'bind') {
                x_admin_show('绑定App用户', '<%=path%>/admin/partner/bindPartnerUser?id='+data.id);
            } else if (obj.event === 'edit') {
                x_admin_show('编辑合伙人', '<%=path%>/admin/partner/partnerUserEdit?id='+data.id);
            }
        });
