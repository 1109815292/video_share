...
public class ZXingCodeUtils {
    ...
    public static void main(String[] args) throws WriterException {
        try {
            ...
            String logoPath = "/home/xya/Desktop/H5娃娃机/logo.png";
            String backgroundPath = "/home/xya/Desktop/H5娃娃机/background.jpg";
            File logoFile = new File(logoPath);
            File backgroundFile = new File(backgroundPath);
            String outPath = "/home/xya/Desktop/aa";
            createQRCode(logoFile, "http://cs.ballq.cn:22080/#/login?invite_player=42", outPath, 400, 400, "", backgroundFile);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static String createQRCode(File logoFile, String content, String outPath,
        ...
        try {
            ...
            return zp.createLogoQRCode(outPath, qrImgWidth, qrImgHeight, bim, logoFile, new LogoConfig(), productName, null);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public static String createQRCode(File logoFile, BufferedImage qrCodeBufferedImage, String outPath,
                                      int qrImgWidth, int qrImgHeight, String productName) {
        try {
            ZXingCodeUtils zp = new ZXingCodeUtils();
            // 如果有文字，则二维码图片高度增加45px
            ...
            // 给二维码图片添加Logo并保存到指定位置，返回base64编码的图片数据字符串
            return zp.createLogoQRCode(outPath, qrImgWidth, qrImgHeight, qrCodeBufferedImage, logoFile, new LogoConfig(), productName, null);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public static String createQRCode(File logoFile, String content, String outPath,
                                      int qrImgWidth, int qrImgHeight, String productName, File backgroundFile) {
        try {
            ZXingCodeUtils zp = new ZXingCodeUtils();
            // 生成二维码bufferedImage图片
            BufferedImage bim = zp.getQRCODEBufferedImage(content, BarcodeFormat.QR_CODE, qrImgWidth, qrImgHeight, zp.getDecodeHintType());
            ...
            // 给二维码图片添加Logo并保存到指定位置，返回base64编码的图片数据字符串
            return zp.createLogoQRCode(outPath, qrImgWidth, qrImgHeight, bim, logoFile, new LogoConfig(), productName, backgroundFile);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
     ...
     */
    public String createLogoQRCode(String outPath, int qrImgWidth, int qrImgHeight, BufferedImage bim, File logoPic, LogoConfig logoConfig, String productName, File backgroudFile) {
        try {
            /**
             * 读取二维码图片，并构建绘图对象
             */
            ...
            if (logoPic != null && logoPic.exists()) {
                ...
                g.drawRoundRect(x, y, widthLogo, heightLogo, 30, 30);
...
//                  outg.drawString(productName, 0, image.getHeight() + (outImage.getHeight() - image.getHeight())/2 + 5 ); //画文字
            ...
            if(backgroudFile != null) {
                bg= ImageIO.read(backgroudFile);//获取背景
...
//                ImageIO.write(image, "png", new File(outPath + "\\" + new Date().getTime() + ".png"));
                if(backgroudFile != null) {
                    ImageIO.write(bg, "png", new File(outPath + "\\" + new Date().getTime() + ".png"));
                }else{
                    ImageIO.write(image, "png", new File(outPath + "\\" + new Date().getTime() + ".png"));
                }
            }
